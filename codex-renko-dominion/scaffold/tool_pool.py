...
def assemble_tool_pool(
    ...
    """Register built-in tools with mode/runtime/permission filtering support."""
    for tool in BUILTIN_TOOLS:
        if read_only_only and not tool.spec.is_read_only:
            continue
        if mode == "execution" and tool.spec.is_destructive:
            continue
        if permission_profile == "strict" and not tool.spec.is_read_only:
            continue
        if runtime_target == "remote" and not tool.spec.is_concurrency_safe:
            continue
