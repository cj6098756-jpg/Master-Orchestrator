...
from scaffold.contracts import SessionState, TurnRequest, TurnResult, utc_now_iso
...
def resume_session(snapshot: dict) -> StoredSession:
    ...
    for t in snapshot["turns"]:
        turns.append(StoredTurn(
            ...
            result=TurnResult(**t["result"]),
            created_at=t["created_at"],
            parent_turn_id=t.get("parent_turn_id"),
            task_refs=t.get("task_refs", []),
        ))
    sess = StoredSession(state, turns, snapshot["messages"], cost, snapshot.get("file_cache_meta", {}))
